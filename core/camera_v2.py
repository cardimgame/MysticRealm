...
class CameraV2:
    def __init__(self, screen_w: int, screen_h: int, world_w: int, world_h: int, *, zoom: float = 1.0):
        ...
        self.dead_frac = (0.38, 0.30)  # fração da visão (w,h) EM MUNDO
    ...
    # --- Propriedades de VISÃO (em coordenadas de mundo) ---
    @property
    def view_w(self) -> float:
        return self.screen_w / max(1e-6, self.zoom)

    @property
    def view_h(self) -> float:
        return self.screen_h / max(1e-6, self.zoom)
    ...
    def set_profile(self, *, dead_frac=None, lookahead_t=None, smooth=None, zoom=None):
        ...
        if lookahead_t is not None:
            self.lookahead_t = lookahead_t
    ...
    def get_rect(self) -> pygame.Rect:
        """Retângulo da área visível EM MUNDO."""
        return pygame.Rect(int(self.x), int(self.y), int(self.view_w), int(self.view_h))
    ...
    def apply_shake(self, power: float = 8.0):
        self.shake_amp = max(self.shake_amp, float(power))
        self.shake_t = 0.0

    def update(self, dt: float, *, focus_px, vel_px=(0.0, 0.0)):
        # 1) foco com lookahead (em mundo)
        fx, fy = focus_px
        vx, vy = vel_px
        fx += float(vx) * self.lookahead_t
        fy += float(vy) * self.lookahead_t

        # 2) deadzone central (em mundo)
        dz_w = self.view_w * self.dead_frac[0]
        dz_h = self.view_h * self.dead_frac[1]
        dz = pygame.Rect(
            int(self.x + (self.view_w - dz_w) / 2),
            int(self.y + (self.view_h - dz_h) / 2),
            int(dz_w), int(dz_h)
        )
