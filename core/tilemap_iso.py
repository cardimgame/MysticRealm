...
class IsoMap:
    ...
    def _get_cam_rect_world(self, camera) -> pygame.Rect:
        ...
        def draw_visible(self, screen: pygame.Surface, camera):
            if not self.layers:
                return
        cam_rect = self._get_cam_rect_world(camera)
        pad_x, pad_y = TILE_W, TILE_H
        view = cam_rect.inflate(pad_x * 2, pad_y * 2)
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                x += self.offset_x
                y += self.offset_y
                if (x + TILE_W > view.left and x < view.right and
                    y + TILE_H > view.top and y < view.bottom):
        ...
        for layer in self.layers:
            ...
            for r, c, x, y in visible_cells:
                ...
                pos = camera.world_to_screen((x, y))
                screen.blit(img, pos)
